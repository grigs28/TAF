...
### 2.6 阶段进度（扫描 → 压缩 → 写入）
- **字段**：
  - `operation_status`：当前阶段的原始描述（例如“压缩文件中”）
  - `operation_stage`：阶段编码（`scan`、`compress`、`copy`、`finalize` 等）
  - `stage_steps`：按顺序返回的阶段列表，包含 `code`、`label`、`state(done/current/pending)`
- **数据来源**：`/api/backup/tasks` / `/api/backup/tasks/{id}` 返回
- **前端展示**：
  - 在运行中卡片顶部显示“当前阶段”
  - 使用彩色徽章表示阶段顺序，state=`done` 显示绿色、`current` 显示蓝色、`pending` 显示灰色
  - 卡片状态变更时仅需要刷新 API 数据即可自动高亮当前阶段
