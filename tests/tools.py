...
class TapeManager:
    ...
    def format_tape_ltfs(self, drive_id=None, volume_label=None, serial=None, eject_after=False):
        ...
        注意: LtfsCmdFormat使用驱动器ID(如0.0.24.0)而不是盘符O:
        """
        print("LTFS格式化磁带...")
        ...
        if serial:
            # 序列号必须是6位大写字母数字
            if len(serial) == 6 and serial.isalnum() and serial.isupper():
                cmd.append(f"/S:{serial}")
            else:
                print("⚠ 序列号必须是6位大写字母数字，已忽略序列号参数")
        
        # 添加卷标参数（如果需要）
        if volume_label:
            # 卷标可以是任意长度，但建议使用有意义的名称
    ...
    def assign_tape_ltfs(self, drive_id=None):
        """
        分配磁带给驱动器并挂载到O:盘
        注意: LtfsCmdAssign使用驱动器ID(如0.0.24.0)和盘符O:作为参数
        """
    ...
    def set_volume_label_via_format(self, label, drive_id=None):
        """
        通过格式化设置卷标
        注意: 卷标可以是任意长度，但建议使用有意义的名称
              序列号必须是6位大写字母数字(如果使用/S参数)
        """
        print(f"通过格式化设置卷标: {label}")
        ...
        # 使用LtfsCmdFormat格式化并设置卷标
        print(f"\n开始格式化磁带并设置卷标...")
        cmd = [self.ltfs_tools['format'], drive_id, f"/N:{label}"]
        ...
        if result and result.returncode == 0:
            ...
            # 格式化后可以重新分配驱动器
            reassign = input("是否要重新分配驱动器? (y/N): ").strip().lower()
            if reassign == 'y':
                self.assign_tape_ltfs(drive_id)
...
def show_itdt_menu():
    ...
    print("7. 完全擦除磁带")
    print("8. 返回主菜单")
    print("\n注意: ITDT的卸载磁带和设置卷标功能已确认无效，已从菜单移除")
    print("=" * 60)
...
def show_ltfs_menu():
    ...
    print("2. 加载磁带 (物理)")
    print("3. 弹出磁带 (物理)")
    print("4. 格式化磁带 (LtfsCmdFormat) - 使用驱动器ID")
    print("5. 格式化磁带 (mkltfs) - 使用设备地址")
    print("6. 取消格式化")
    print("7. 检查磁带完整性")
    print("8. 回滚磁带")
    print("9. 完整挂载流程 (加载->格式化->挂载O:盘)")
    print("10. 返回主菜单")
    print("\n注意:")
    print("- LtfsCmdFormat使用驱动器ID(如0.0.24.0)而不是盘符O:")
    print("- 分配和卸载磁带功能在某些环境下可能不稳定")
    print("=" * 60)
...
def handle_set_volume_label(manager):
    """处理卷标设置操作"""
    clear_screen()
    print("=" * 60)
    print("                 设置磁带卷标")
    print("=" * 60)
    print("ℹ 注意:")
    print("- ITDT工具无法设置卷标，只能通过格式化设置")
    print("- 卷标可以是任意长度，建议使用有意义的名称")
    print("- 序列号必须是6位大写字母数字(如果使用/S参数)")
    print("1. 通过LTFS格式化设置卷标 (会擦除所有数据!)")
    print("2. 返回主菜单")
    print("=" * 60)
    ...
    if choice == '1':
        ...
        if label:
            if manager.set_volume_label_via_format(label):
                print("✓ 通过格式化设置卷标成功")
            else:
                print("✗ 通过格式化设置卷标失败")
    elif choice == '2':
    ...
    try:
        ...
        while True:
            ...
            if choice == '0':
                ...
                handle_set_volume_label(manager)
            else:
                print("无效选择")
                input("\n按回车键继续...")
