...
async def get_compressed_files_count_redis(backup_set_db_id: int) -> int:
    ...
    try:
        ...
        # 统计 is_copy_success = '1' 且 chunk_number IS NOT NULL 的文件数
        compressed_count = 0
        for file_id_bytes in file_ids:
            try:
                ...
                # 检查文件类型、is_copy_success 状态和 chunk_number
                file_type = await redis.hget(file_key, 'file_type')
                is_copy_success = await redis.hget(file_key, 'is_copy_success')
                chunk_number = await redis.hget(file_key, 'chunk_number')
                
                # 只统计真正压缩完成的文件（chunk_number IS NOT NULL）
                if (file_type == 'file' 
                    and is_copy_success in ('1', 'True', 'true')
                    and chunk_number is not None 
                    and chunk_number != ''):
                    compressed_count += 1
            except (ValueError, TypeError):
                continue
