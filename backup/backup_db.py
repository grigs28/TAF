    ...
    async def update_scan_progress(
        ...
        try:
            ...
            if is_opengauss():
                conn = await get_opengauss_connection()
                try:
                    # 根据操作状态决定 total_files 的值
                    # 如果是扫描阶段，使用 backup_task.total_files（压缩包数量，初始为0）
                    # 如果是压缩/写入阶段，使用 valid_count（压缩包数量）
                    if operation_status and ('[压缩' in operation_status or '[写入' in operation_status or '[完成' in operation_status):
                        # 压缩/写入/完成阶段：使用 valid_count 作为压缩包数量
                        total_files_value = valid_count
                    else:
                        # 扫描阶段：使用 backup_task.total_files（压缩包数量，初始为0）
                        total_files_value = getattr(backup_task, 'total_files', 0) or 0
                    ...
                    if operation_status:
                        ...
                        await conn.execute(
                            ...
                            total_bytes,  # total_bytes: 所有扫描到的文件总数
                            processed_bytes,
                            compressed_bytes,
                            result_summary_json,
                            new_desc,
