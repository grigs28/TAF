...
class BackupDB:
                ...
                async with get_opengauss_connection() as conn:
                    ...
                    if hasattr(actual_conn, 'commit'):
                        try:
                            ...
                            logger.info(f"提交备份集创建事务失败（可能已自动提交）: {commit_err}")
        ...
        try:
            ...
            async with get_opengauss_connection() as conn:
                try:
                    ...
                    logger.info(f"⚠️ 数据库连接或查询失败，跳过保存文件信息: {str(db_conn_error)}")
                    # 数据库错误不影响备份流程，继续执行
    ...
    async def fetch_pending_files_grouped_by_size(
        ...
        async with get_opengauss_connection() as conn:
            ...
            # 如果 start_from_id 小于第一个未压缩文件ID - 1，说明可能跳过了某些文件，应该从第一个文件开始
            # 注意：start_from_id = first_pending_id - 1 是正常的（查询条件 id > start_from_id 能包含第一个文件）
            if start_from_id > 0 and start_from_id < first_pending_id - 1:
                logger.info(
            ...
            first_skipped_file_id = None  # 记录第一个跳过的文件ID（用于情况2.3）
            first_skipped_file_info = None  # 记录第一个跳过的文件信息（用于最后一批时添加）
            while not should_stop:
                ...
                for row in rows:
                    ...
                    # 第二原则：文件组累积处理
                    # 只有文件 <= max_file_size - 5% 的文件才会进入累积逻辑
                    new_group_size = current_group_size + file_size

                    # 情况2.1：new_group_size <= max_file_size - 5%，继续累积文件
                    ...
                    if new_group_size > max_group_size:
                        # 记录第一个跳过的文件ID和文件信息（用于下一批检索的起始点，或最后一批时添加）
                        if first_skipped_file_id is None:
                            first_skipped_file_id = row['id']
                            first_skipped_file_info = file_info  # 保存文件信息，用于最后一批时添加
                            logger.info(
                                f"[第二原则] 跳过第一个大文件：加入后组大小 {format_bytes(new_group_size)} > {format_bytes(max_group_size)}，"
                                f"文件 {file_info['name']} ({format_bytes(file_size)})，"
                                f"记录第一个跳过文件ID={first_skipped_file_id}，下一批将从该文件开始检索"
                            )
                        else:
                            logger.debug(
                                f"[第二原则] 跳过大文件：加入后组大小 {format_bytes(new_group_size)} > {format_bytes(max_group_size)}，"
                                f"文件 {file_info['name']} ({format_bytes(file_size)})，跳过此文件继续累积"
                            )
        ...
        if all_files:
            ...
            # 如果文件组大小不够，且这一批文件检索完（没有更多文件了），尝试添加跳过的文件
            if current_group_size < min_group_size and first_skipped_file_info is not None:
                # 检查添加跳过的文件后是否超过容差上限
                skipped_file_size = first_skipped_file_info.get('size', 0)
                new_size_with_skipped = current_group_size + skipped_file_size
                
                if new_size_with_skipped <= max_group_size:
                    # 添加跳过的文件后不超过容差上限，添加并返回
                    current_group.append(first_skipped_file_info)
                    current_group_size = new_size_with_skipped
                    logger.info(
                        f"[openGauss优化] 这一批文件检索完，文件组大小 {format_bytes(current_group_size - skipped_file_size)} "
                        f"< {format_bytes(min_group_size)}，添加之前跳过的文件 "
                        f"({format_bytes(skipped_file_size)})，"
                        f"新文件组大小: {format_bytes(new_size_with_skipped)}"
                    )
                    # 更新 last_processed_id 为跳过文件的ID
                    last_processed_id = first_skipped_file_id
                    first_skipped_file_id = None  # 已添加，清除跳过标记
                    first_skipped_file_info = None
                else:
                    # 添加跳过的文件后超过容差上限，不添加
                    logger.info(
                        f"[openGauss优化] 这一批文件检索完，文件组大小 {format_bytes(current_group_size)} "
                        f"< {format_bytes(min_group_size)}，但添加跳过的文件后会超过容差上限 "
                        f"({format_bytes(new_size_with_skipped)} > {format_bytes(max_group_size)})，"
                        f"不添加跳过的文件，下一批将从跳过文件ID={first_skipped_file_id}开始检索"
                    )
            
            # 如果达到容差范围，直接返回
            if current_group_size >= min_group_size:
                # 确定返回的 last_processed_id：如果有跳过的文件，使用第一个跳过的文件ID作为下一批的起始点
                return_last_processed_id = last_processed_id
                if first_skipped_file_id is not None:
                    # 使用第一个跳过的文件ID作为下一批的起始点（id > first_skipped_file_id - 1 即 id >= first_skipped_file_id）
                    return_last_processed_id = first_skipped_file_id - 1
                    logger.info(
                        f"[openGauss优化] 达到容差范围内：{format_bytes(current_group_size)} "
                        f"({size_ratio*100:.1f}% of 目标，≥ {format_bytes(min_group_size)})，"
                        f"有跳过的文件，下一批将从第一个跳过文件ID={first_skipped_file_id}开始检索"
                    )
                else:
                    logger.info(
                        f"[openGauss优化] 达到容差范围内：{format_bytes(current_group_size)} "
                ...
                # 返回文件组和最后处理的文件ID
                return ([current_group], return_last_processed_id)
        ...
        if current_group_size < min_group_size and scan_status != 'completed' and retry_count < max_retries:
            ...
            # 如果 current_group 不为空，说明已经检索到文件但大小不够，需要记录最后处理的文件ID
            if current_group and last_processed_id > 0:
                logger.info(
                    f"[openGauss优化] 已检索到 {len(current_group)} 个文件但大小不够，"
                    f"最后处理的文件ID: {last_processed_id}，下次查询将从 id > {last_processed_id} 开始"
                )
                # 返回空列表和最后处理的文件ID，继续等待直到满足大小要求
        ...
        if current_group_size < min_group_size:
            # 如果文件组大小不够，且这一批文件检索完（没有更多文件了），尝试添加跳过的文件
            if first_skipped_file_info is not None:
                # 检查添加跳过的文件后是否超过容差上限
                skipped_file_size = first_skipped_file_info.get('size', 0)
                new_size_with_skipped = current_group_size + skipped_file_size
            ...
            if current_group_size < min_acceptable_group_size:
                ...
                if first_skipped_file_id is not None:
                    return_last_processed_id = first_skipped_file_id - 1
                return ([], return_last_processed_id)
            ...
            if first_skipped_file_id is not None:
                return_last_processed_id = first_skipped_file_id - 1
                logger.info(
                    f"[openGauss优化] 强制返回文件组，有跳过的文件，下一批将从第一个跳过文件ID={first_skipped_file_id}开始检索"
                )
            return ([current_group], return_last_processed_id)
        ...
        if current_group_size >= min_group_size:
            ...
            if first_skipped_file_id is not None:
                return_last_processed_id = first_skipped_file_id - 1
                logger.info(
                    f"[openGauss优化] 达到容差范围内，有跳过的文件，下一批将从第一个跳过文件ID={first_skipped_file_id}开始检索"
                )
            # 返回文件组和最后处理的文件ID
            return ([current_group], return_last_processed_id)
        else:
    ...
    async def update_scan_status(self, backup_task_id: int, status: str):
        ...
        if is_redis():
            ...
            async with get_opengauss_connection() as conn:
                ...
                # 关键节点：立即提交事务，确保状态更新对其他线程可见
                try:
                    await conn.commit()
                except Exception as commit_error:
                    logger.error(
                        f"[update_scan_status] ⚠️ 提交扫描状态更新事务失败: {commit_error}"
                    )
                    raise
        else:
            # SQLite 版本
    ...
    async def get_compressed_files_count(self, backup_set_db_id: int) -> int:
        ...
        Returns:
            已压缩文件数（is_copy_success = TRUE 的文件数）
        """
        try:
            from utils.scheduler.db_utils import is_opengauss, is_redis, get_opengauss_connection
            from utils.scheduler.sqlite_utils import is_sqlite
